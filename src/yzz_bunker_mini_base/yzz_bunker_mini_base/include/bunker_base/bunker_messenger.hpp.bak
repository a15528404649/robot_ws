...
class BunkerMessenger {
 public:
  ...
  void PublishStateToROS() {
    ...
    double dt = static_cast<double>(current_time_.nanoseconds() - last_time_.nanoseconds()) * 1e-9;
    ...
    for (int i = 0; i < 2; ++i) {
      // BUNKER MINI reports its two motors with CAN IDs 0x252/0x253,
      // which ugv_sdk stores at indices 1 and 2.
      const int sdk_index = i + 1;

      status_msg.actuator_states[i].motor_id = i;
      status_msg.actuator_states[i].rpm =
          actuator.actuator_hs_state[sdk_index].rpm;
      status_msg.actuator_states[i].current =
          actuator.actuator_hs_state[sdk_index].current;
      status_msg.actuator_states[i].pulse_count =
          actuator.actuator_hs_state[sdk_index].pulse_count;

      status_msg.actuator_states[i].driver_voltage =
          actuator.actuator_ls_state[sdk_index].driver_voltage;
      status_msg.actuator_states[i].driver_temperature =
          actuator.actuator_ls_state[sdk_index].driver_temp;
      status_msg.actuator_states[i].motor_temperature =
          actuator.actuator_ls_state[sdk_index].motor_temp;
      status_msg.actuator_states[i].driver_state =
          actuator.actuator_ls_state[sdk_index].driver_state;
    }
