...
class Detect_AFPN(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        super().__init__()
        channel = 256
        export = (channel, channel, channel)
        ...
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in export)  # output conv
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if isinstance(self, Segment_AFPN):  # (boxes + masks)
...
class Segment_AFPN(Detect_AFPN):
    ...
    def __init__(self, nc=80, anchors=(), nm=32, npr=256, ch=(), inplace=True):
        ...
        self.detect = Detect_AFPN.forward
